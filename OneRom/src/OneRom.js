...
//                                Control.value=value;
    ...
    this.RestartComponents = function () {
        //Room.Funciones = [];
    ...
    function EvaluarPropiedades(Control) {
        ...
        for (var atributos of Control.attributes) {
            if (atributos.name.includes("[")) {
                //------------------------------Programar atributos-----------------
                var StringSinCorchetes = atributos.name.split("[").join("");
                StringSinCorchetes = StringSinCorchetes.split("]").join("");
                var codigo = "Elemto." + ValidaAtributosEspeciales(StringSinCorchetes) + " = " + atributos.value;
                ...
                ArrayAtributosBorrar.push(atributos.name);
    ...
    function ValidaAtributosEspeciales(Atributo) {
        var NombreAtributo = "";
    ...
    function construirPropiedad(objeto, NombreProp) {
        if (Array.isArray(objeto)) {
            const iterator = objeto.keys();
            for (var key of iterator) {
                construirPropiedad(objeto[key], NombreProp + "[" + key + "]");
            }
        }
...
//        console.log(NombrePropiedadSinDoor);
            eval(ejecutable);
            DoorTemp.watch(NombrePropiedadFinal, function (variable, prop, valor) {
                window.setTimeout(function () {
                    for (var funcion of Room.FuncionesBinding) {
...
//        ObjectsHtmlRadio = undefined;
    ...
    Object.defineProperty(window, "Hmensajeinfo3W", {get: function () {
            return Hmensajeinfo;
        },
        set: function (value) {
